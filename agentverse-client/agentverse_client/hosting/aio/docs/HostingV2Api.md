...
async with agentverse_client.hosting.aio.ApiClient(configuration) as api_client:
    ...
    address = 'address_example' # str | 
    no_cache = False # bool |  (optional) (default to False)
    duplicate_hosted_agent_options = agentverse_client.hosting.aio.DuplicateHostedAgentOptions() # DuplicateHostedAgentOptions |  (optional)

    try:
        # Duplicate Hosted Agent
        api_response = await api_instance.duplicate_hosted_agent_v2(address, no_cache=no_cache, duplicate_hosted_agent_options=duplicate_hosted_agent_options)
