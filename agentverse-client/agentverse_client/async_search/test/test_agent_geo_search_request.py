# coding: utf-8

"""
    FastAPI

    No description provided (generated by Openapi Generator https://github.com/openapitools/openapi-generator)

    The version of the OpenAPI document: 0.1.0
    Generated by OpenAPI Generator (https://openapi-generator.tech)

    Do not edit the class manually.
"""  # noqa: E501


import unittest

from agentverse_client.async_search.models.agent_geo_search_request import AgentGeoSearchRequest

class TestAgentGeoSearchRequest(unittest.TestCase):
    """AgentGeoSearchRequest unit test stubs"""

    def setUp(self):
        pass

    def tearDown(self):
        pass

    def make_instance(self, include_optional) -> AgentGeoSearchRequest:
        """Test AgentGeoSearchRequest
            include_optional is a boolean, when False only required
            params are included, when True both required and
            optional params are included """
        # uncomment below to create an instance of `AgentGeoSearchRequest`
        """
        model = AgentGeoSearchRequest()
        if include_optional:
            return AgentGeoSearchRequest(
                filters = agentverse_client.async_search.models.agent_filters.AgentFilters(
                    state = [
                        'active'
                        ], 
                    category = [
                        'fetch-ai'
                        ], 
                    agent_type = [
                        'hosted'
                        ], 
                    protocol_digest = [
                        ''
                        ], 
                    has_location = True, 
                    has_readme = True, 
                    n_interactions = '1k', 
                    tags = [
                        ''
                        ], ),
                sort = 'relevancy',
                direction = 'asc',
                search_text = '',
                offset = 56,
                limit = 56,
                geo_filter = agentverse_client.async_search.models.agent_geo_filter.AgentGeoFilter(
                    latitude = -90.0, 
                    longitude = -180.0, 
                    radius = 1.337, ),
                search_id = '',
                source = ''
            )
        else:
            return AgentGeoSearchRequest(
                geo_filter = agentverse_client.async_search.models.agent_geo_filter.AgentGeoFilter(
                    latitude = -90.0, 
                    longitude = -180.0, 
                    radius = 1.337, ),
        )
        """

    def testAgentGeoSearchRequest(self):
        """Test AgentGeoSearchRequest"""
        # inst_req_only = self.make_instance(include_optional=False)
        # inst_req_and_optional = self.make_instance(include_optional=True)

if __name__ == '__main__':
    unittest.main()
