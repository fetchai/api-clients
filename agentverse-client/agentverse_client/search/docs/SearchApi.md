...
[**get_agent**](SearchApi.md#get_agent) | **GET** /v1/search/agents/{address} | Get Agent
...
with agentverse_client.search.ApiClient(configuration) as api_client:
    ...
    try:
        # Get Agent
        api_response = api_instance.get_agent(address, contract=contract)
        print("The response of SearchApi->get_agent:\n")
        pprint(api_response)
    except Exception as e:
        print("Exception when calling SearchApi->get_agent: %s\n" % e)
```
