...
with agentverse_client.search.ApiClient(configuration) as api_client:
    # Create an instance of the API class
    api_instance = agentverse_client.search.AgentsApi(api_client)
    multi_seo_evaluation_request = agentverse_client.search.MultiSEOEvaluationRequest() # MultiSEOEvaluationRequest | 

    try:
        # Start Multi Seo Eval
        api_instance.start_multi_seo_eval(multi_seo_evaluation_request)
    except Exception as e:
